...
def generate_envrc_template(
    ...
    Args:
        ...
        monorepo_root: Base directory containing ``confexts/``. When
            ``None``, resolved by walking up from *cwd*. Falls back to a
            placeholder string if not found.
...
def generate_ssh_config(
    ...
    # Other hosts: private IP + ProxyJump through jumphost (if jumphost exists)
    has_jumphost = "jumphost" in hosts
    for role in sorted(hosts):
        ...
        if has_jumphost:
            host_lines.append(f"  ProxyJump               {env_name}-jumphost-{ordinal}")
        host_lines.append("")
...
def create_ssh_config(
    ...
    *,
    ...
    # Create empty known_hosts (write_text to clear contents on --force)
