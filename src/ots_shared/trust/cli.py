...
(``lots``, ``pots``, ...) parameterize user-facing error wording with its own
tool name. A module-level ``app = make_trust_app()`` is also exposed for the
no-tool-name default and for direct module-import use.
"""
...
def make_trust_app(tool_name: str | None = None) -> cyclopts.App:
    """Build the shared trust subapp.

    Parameters
    ----------
    tool_name:
        Parent CLI name (e.g. ``"lots"``, ``"pots"``) used in user-facing
        error messages. When ``None``, messages omit the tool name and
        point operators at the generic init wording.
    """
    app = cyclopts.App(
        name="trust",
        help=(
            "Inspect and manage trust material under .trust/. "
            "Subcommands: fingerprints, list, prune."
        ),
    )

    if tool_name:
        init_hint = f"run `{tool_name} init` to materialize trust material"
    else:
        init_hint = "run the init command for your environment to materialize trust material"
    ...
    def _require_trust_dir() -> Path:
        ...
        if not trust_dir.is_dir():
            print(f"Error: {trust_dir} not found; {init_hint}.", file=sys.stderr)
            sys.exit(1)
        return trust_dir
...
# Default module-level subapp for the no-tool-name case. Downstream parent
# CLIs that want their tool name in error wording should call
# ``make_trust_app("lots")`` (etc.) instead of importing this directly.
app = make_trust_app()
